...
model = NeRFModel.load_from_checkpoint(parameters.load_from_ckpt, log_dir=parameters.log_dir, exp_name=parameters.exp_name)
trainer = pl.Trainer(
...
predictions = trainer.predict(model, DataLoader(test_dataset, batch_size=1, shuffle=False),
                              ckpt_path=parameters.load_from_ckpt)
